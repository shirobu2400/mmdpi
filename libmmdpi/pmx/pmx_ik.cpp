...
int mmdpiPmxIk::ik_execute( MMDPI_BONE_INFO_PTR bone, MMDPI_PMX_BONE_INFO_PTR pbone, int bone_index )
{
	...
	const float	bottom_noise = 1e-3f;
	...
	for( uint j = 0; j < iteration_num; j ++ )
	{
		...
		for( int i = 0; i < ik_link_num; i ++ )
		{
			...
			mmdpiMatrix		local_matrix		= mmdpiBone::get_global_matrix( target_bone );
			mmdpiMatrix		inv_matrix		= local_matrix.get_inverse();
			...
			// ローカル座標系へ変換
			local_effect_pos = inv_matrix * effect_pos_base;
			local_target_pos = inv_matrix * target_pos_base;
			...
			// 反映
			target_bone->temp_matrix = rotation_matrix * target_bone->temp_matrix;
		...
		if( rotation_distance < bottom_noise )
			break;
	}
