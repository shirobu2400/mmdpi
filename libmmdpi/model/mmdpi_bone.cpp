...
void mmdpiBone::make_local_matrix( void )
{
	for( uint i = 0; i < this->mmdpiBone::bone_num; i ++ )
		this->bone[ i ].local_matrix = this->bone[ i ].offset_mat * this->bone[ i ].matrix;
}

// マトリックス生成
int mmdpiBone::global_matrix( void )
{
	return this->make_matrix( &this->bone[ 0 ], 0x00 );
}
...
int mmdpiBone::make_matrix( MMDPI_BONE_INFO_PTR my_bone, const mmdpiMatrix* offset )
{
	...
	if( my_bone->first_child )
		this->make_matrix( my_bone->first_child, &my_bone->matrix );
	if( my_bone->sibling )
		this->make_matrix( my_bone->sibling, offset );
...
mmdpiMatrix mmdpiBone::make_global_matrix( int index )
{
	if( this->bone[ index ].parent == 0x00 )
		return this->bone[ index ].bone_mat * this->bone[ index ].delta_matrix;
	return this->bone[ index ].bone_mat * this->bone[ index ].parent->matrix * this->bone[ index ].delta_matrix;
}
...
void mmdpiBone::refresh_bone_mat( void )
{
	...
	{
		this->bone[ i ].bone_mat = this->bone[ i ].init_mat;
...
int mmdpiBone::create_physical_info( void )
{
	...
	for( uint i = 0; i < this->rigidbody_count; i ++ )
	{
		...
		if( j < bone_num )
			bone_matrix_l = &this->bone[ j ].matrix;
		else
		{
			bone_matrix_l = &this->bone[ center_index ].matrix;
			j = center_index;
		}
		...
		this->physics_sys[ i ].rbInfo.kinematicMatrix = &this->bone[ j ].matrix;
		...
		btTransform btt = this->matrix_to_btTrans( bone_pos, this->physics[ i ].rot );
	...
	{
		// 点と点のジョイント
		btTransform			trans = this->matrix_to_btTrans( joint[ i ].pos, joint[ i ].rot );
		...
		this->create_joint_p2p( joint[ i ].a_index, joint[ i ].b_index, &trans, &j_info );
	}
